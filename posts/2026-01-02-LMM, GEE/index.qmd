...
$$
여기서 $\mathbf{G}$는 환자 간의 이질성(Between-subject variability)을 설명하는 공분산 행렬이며, $\mathbf{R}_i$는 환자 내 잔차의 공분산 행렬이다.
가장 단순한 형태인 조건부 독립(Conditional Independence) 가정 하에서는 $\mathbf{R}_i = \sigma^2 \mathbf{I}_{n_i}$가 된다. 
...
**- exchangeable이 안정적으로 적합되고 $\hat{\rho}$ 추정이 합리적인 경우 경우: **
exchangeable 상관구조를 사용한다.

**- exchangeable이 불안정하거나(수렴/경고/극단적 $\hat{\rho}$) 상관 추정 정보가 부족한 경우: **
주 분석을 independence로 두고, 가능하다면 exchangeable 시도 결과를 민감도 분석(또는 부록)으로만 언급한다.
